...
import { bypassMode } from "./index.js";
...
/* WALLET */
var privateKeys = ['88c7dedee9b3db10a2499dfb8f576e4ee3217a76c1c887de6274be041540d3bd', 'e66b10630e488d4efe9e8199334b7992387e65d4fc5e51f7f44bd88abdbaffe0'];
...
let contracts = ['0x237AB82C8a7a3EaD975B0B9Fee13c05d792eeC59', '0x8Cdce37B80809789b4AdFB42676E83eBBCcb0581', '0xA9EdA3126C14825f7952703d78b4eBfc1516Fb39', '0xF171817FC293136117fde7C74e74b75DA08332a1', '0x97AF9b7d2bff52b9e0C9C4B437A148A56658e763', '0xFf4115829090e0b552044f5840390ceE1f2B75eC'];
...
export async function getTotalSupply() {
    return await cTOKEN.totalSupply_();
}
...
export async function createDelta(inputParams) {
    ...
    let ID = await cLOCKER.nextID();
    let r = await cLOCKER.createDelta(inputParams[0], inputParams[1], paramValues, returnTypes, await encodeParams(inputParams[1], paramTypes, paramValues), await getGasObj());
    await r.wait();
    console.log(r);
    return ID;
}
...
export async function encodeParams(functionName, paramTypes, paramValues) {
...
//EVENT
export async function subscribe(inputParams) {
    ...
    let paramTypesList = paramTypes.split("_indexed").join(' indexed').split(",");
    ...
    let outputs = await contract.queryFilter(getEventFilterInput(contractAddress, params, bytesList), -900);
    console.log(outputs);

    if (outputs.length == 0 && !bypassMode) {
        alert("WARNING: recent events not found. Type 'b' to enter bypass mode.");
    } else {
...
function getEventFilterInput(contractAddress, abi, bytesList) {
...
export async function testAll() {
    ...
    try {
        ...
        // await subscribe([cLOCKER.address, 'CreateDelta', 'address_indexed,address_indexed,uint_indexed', 'token,contract,ID', cTOKEN.address]);
        // await subscribe([cLOCKER.address, 'ExecuteDelta', 'address_indexed,address_indexed,uint_indexed,bytes', 'token,contract,ID,bytes', cTOKEN.address + ',' + cTOKEN.address]); 
        // await subscribe([cAFFILIATE.address, 'SetupAffiliate', 'address_indexed,address_indexed', 'token,wallet', cTOKEN.address]);
        await subscribe([cSTAKE.address, 'Stake', 'address_indexed,uint', 'wallet, wei', wallet._address]);
        // await subscribe([cSALE.address, 'NewSale', 'address_indexed,uint_indexed,uint', 'tokenSale,%LP,duration', cTOKEN.address]);
        // await subscribe([cSALE.address, 'EndSale', 'address_indexed,uint,uint', 'tokenSale,weiToLiquidity,weiToWallet', cTOKEN.address]);
        // await subscribe([cSALE.address, 'UnlockLiquidity', 'address_indexed,uint,uint', 'tokenSale,weiToWallet,weiFee', cTOKEN.address]);
        // await subscribe([cAFFILIATE.address, 'AffiliateDistribution', 'address_indexed,address_indexed,address_indexed,uint', 'token,affiliate,wallet,weiIn', cTOKEN.address]);
        // await subscribe([cEVENT.address, 'SubscribeEvent', 'address_indexed,address_indexed', 'contract,wallet', 'null,' + wallet._address]);
